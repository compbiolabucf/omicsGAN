...
print('miRNA_'+str(serial)+'_'+str(update))
...
adj = pd.read_csv('rand_net'+str(serial)+'.csv',delimiter=',',header=None)
adj=np.array(adj).astype(np.float32)
...
for epoch in range(num_epochs):
    for n, (real_samples, _) in enumerate(miRNA_train_loader):
        ...
        elif epoch % 10000==9999:
            prediction1(generated_samples.cpu().detach().numpy(),sample_name,epoch)
            filename = 'miRNA_'+str(serial)+'_'+str(update)+'_'+str(epoch)+'.csv'
