...
def prediction1(mRNA_value,mRNA_sample,GAN_epoch):
  ...
  data=pd.read_csv('ov_tcga_clinical_data.tsv',delimiter='\t',usecols=[2,8])
  data=np.array(data)
  label_name=data[:,0].astype(str)
  cancer_stage=data[:,1].astype(str)
  ind1=np.where(cancer_stage=='nan')[0]
  ind2=np.where(cancer_stage=='Stage X')[0]
  ind=np.union1d(ind1,ind2)
  label_name=np.delete(label_name,ind)
  cancer_stage=np.delete(cancer_stage,ind)

  labels=np.zeros(np.shape(label_name))
  ind3=np.where(cancer_stage=='Stage IV')[0]
  ind4=np.where(cancer_stage=='Stage IIIC')[0]
  ind=reduce(np.union1d, (ind3,ind4))#ind1,ind2,
  labels[ind]=1
...
for epoch in range(num_epochs):
    for n, (real_samples, _) in enumerate(miRNA_train_loader):
        ...
        latent_value = torch.matmul(mRNA_train_data.t(),adj.t())
        ...
        if epoch ==0:
            ...
            filename = 'miRNA_'+str(update)+'_'+str(epoch)+'.csv'
