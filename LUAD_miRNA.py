...
if len(sys.argv)!=2:
  print('input: code.py update')# gpu prediction classifier \n prediction: survival = 0 , stage =1 \n classifier: rbf/linear/RF')
  sys.exit()

pred = 0#int(sys.argv[3])
clf_ind="linear"#sys.argv[4]
...
def FILENAME(update,clf_ind,pred):
  if pred==1:
    filename = '/home/tahmed/OmicsGAN/LUAD/stage_miRNA_'+clf_ind+'_'+str(update)+'.csv'
  else:
    filename = '/home/tahmed/OmicsGAN/LUAD/miRNA_all'+clf_ind+'_'+str(update)+'.csv'
...
def SVM(X_train, y_train, X_test, y_test,clf_ind):
...
def prediction(mRNA_value,GAN_epoch,y,clf_ind):
  ...
  for col in np.array([0]):
    ...
    for i in range(trial):
      ...
      auc = SVM(X_train, y_train, X_test, y_test,clf_ind)
      AUC.append(auc)
    AUC_all.append(AUC)
...
if update>1:
  if pred==1:
    mRNA_file_name = '/home/tahmed/OmicsGAN/LUAD/stage_mRNA_'+clf_ind+'_'+str(update-1)+'.csv'
    miRNA_file_name = '/home/tahmed/OmicsGAN/LUAD/stage_miRNA_'+clf_ind+'_'+str(update-1)+'.csv'
  elif pred==0:
    mRNA_file_name = '/home/tahmed/OmicsGAN/LUAD/mRNA_all'+clf_ind+'_'+str(update-1)+'.csv'
    miRNA_file_name = '/home/tahmed/OmicsGAN/LUAD/miRNA_all'+clf_ind+'_'+str(update-1)+'.csv'
...
#miRNA = pd.read_csv('miRNA_linear_4.csv',delimiter=',')
...
for epoch in range(num_epochs):
    for n, (real_samples, _) in enumerate(miRNA_train_loader):
        ...
        if epoch ==0:
            #auc = prediction(real_samples.cpu().detach().numpy(),y,epoch,clf_ind)
            auc = prediction(real_samples.cpu().detach().numpy(),epoch,y,clf_ind)
            
        elif epoch % 100==99:
            auc = prediction(generated_samples.cpu().detach().numpy(),epoch,y,clf_ind)
            #auc = prediction(generated_samples.cpu().detach().numpy(),y,epoch,clf_ind)
            filename = FILENAME(update,clf_ind,pred)
