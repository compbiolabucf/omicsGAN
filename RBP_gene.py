...
for epoch in range(num_epochs):
    for n, (real_samples, _) in enumerate(mRNA_train_loader):
        ...
        if epoch ==0:
            ...
            #sys.exit()
            #auc = prediction(latent_value.cpu().detach().numpy(),y,epoch)
            #sys.exit()
            #print('pass')
        #elif epoch == 299:
         #   prediction(generated_samples.detach().numpy(),sample_name,epoch)
          #  np.savetxt('mRNA_300.csv',real_samples.detach().numpy(),delimiter=',',fmt='%s')
        elif epoch % 100==99:
            auc = prediction(generated_samples.cpu().detach().numpy(),y,epoch)
        
            if pred==1:
              filename = '/home/tahmed/OmicsGAN/LUAD/stage_mRNA_'+str(update)+'.csv'
            else:
              filename = '/home/tahmed/OmicsGAN/LUAD/mRNA_'+str(update)+'.csv'
